...
import org.kokakiwi.ssell.fortressassault.FAGizmoHandler.FAGizmo;
import org.kokakiwi.ssell.fortressassault.FortressAssault.FAPlayer;
...
public class FAPlayerListener extends PlayerListener {
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		if (thisPlayer != null) {
			if (plugin.phase != 0) {
				// this works for restoring items when player goes offline, but
				// there are issues when player come back to restore/equip them
				/*
				 * plugin.returnInventory(thisPlayer); //force save of offline
				 * player. CraftWorld cWorld = (CraftWorld)thisPlayer.world;
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		// check if player that join is in the game. getFAPlayer will update
		// their entityid properly.
		Player player = event.getPlayer();
		FAPlayer thisPlayer = plugin.getFAPlayer(player);
		if (thisPlayer != null) {
			// player came back during the game, restore their items and give
			// them the proper equipment.
			player = plugin.getServer().getPlayer(player.getDisplayName());
			if (plugin.phase != 0) {
				// not sure why but if a player dies then disconnects this will
				// cause them to die when they log back in.
	...
	public void delayedCheck(PlayerRespawnEvent event) {
		...
		FAGizmo gizmo = plugin.getGizmoHandler().getGizmo(thisPlayer.team);
		Block gizmoBlock = gizmo.block;
