...
public class FAPvPWatcher {
		...
		if ((FAkiller != null) && (FAkilled != null)) {
			...
			if (FAkiller.team == FAkilled.team) {
				parent.getServer().broadcastMessage(
						parent.getTeamColor(FAkiller.team) + FAkiller.name
								+ " committed treason on " + FAkilled.name);
				FAkiller.deaths += 1;
			} else {
				parent.getServer().broadcastMessage(
						parent.getTeamColor(FAkiller.team) + FAkiller.name
								+ " killed " + FAkilled.name);
	...
	public void destructionEvent(Player player) {
		...
		if (thisPlayer != null) {
			parent.getServer().broadcastMessage(
					parent.getTeamColor(thisPlayer.team) + thisPlayer.name
							+ " destroyed the Gizmo!");
