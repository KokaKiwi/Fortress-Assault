...
import org.kokakiwi.ssell.fortressassault.FortressAssault.Team;
...
public class FAGizmoHandler {
	...
	public boolean addGizmo(FAPlayer thisPlayer, Block block) {
		...
		for (int i = 0; i < gizmoList.size(); i++) {
			if (gizmoList.get(i).team.equals(gizmo.team)) {
				thisPlayer.player.sendMessage(ChatColor.DARK_RED
		...
		plugin.getServer().broadcastMessage(
				plugin.getTeamColor(thisPlayer.team)
						+ thisPlayer.team.toString()
	...
	public void gizmoHit(FAPlayer player, Block block) {
		...
		if (player.team.equals(gizmo.team)) {
			...
			if (gizmo.destructing) {
				...
				plugin.getServer().broadcastMessage(
						plugin.getTeamColor(player.team) + player.name
								+ " saved the " + player.team.toString()
								+ " Gizmo!");
			}
		} else {
			// Opposing team
			if (!gizmo.destructing) {
				...
				plugin.getServer().broadcastMessage(
						plugin.getTeamColor(player.team) + player.name
								+ " attacked the " + gizmo.team.toString()
								+ " Gizmo!");
	...
	public FAGizmo getGizmo(Team team)
	{
		for(FAGizmo gizmo : gizmoList)
		{
			if(gizmo.team == team)
			{
				return gizmo;
